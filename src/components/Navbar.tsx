...
export default function Navbar({ transparent = false }: NavbarProps) {
  ...
  const { logoutPrivy, getSolanaWalletAddress, isPrivyAuthenticated } = usePrivyAuth();
        ...
        <div className="flex items-center gap-2">
          ...
          {walletDisplay ? (
            <div 
              ...
              onClick={() => {
                // Navigate to profile page with wallet tab active
                router.push(`/profile/${user.id}?tab=wallet`);
              }}
              title="View wallet"
            >
              ...
              <span className="text-white text-sm font-mono">
                {`${walletDisplay.address.slice(0, 4)}...${walletDisplay.address.slice(-4)}`}
              </span>
            </div>
          ) : authMethod !== 'privy' && (
